...
module.exports = async function ({ ethers, getNamedAccounts, deployments }: any) {
    ...
    const STRATEGY_BEACON = await ethers.getContract("HarvestStrategyBeacon")
    const { address } = await deploy("HarvestEthFactory", {
      from: deployer,
      args: [
              HARVEST_REWARD_VAULT, 
              HARVEST_REWARD_POOL, 
    ...
    // console.log('[GRANT_ROLE:HarvestEthFactory] receiptFactory grant token creator ', receipt.hash)
 
  }
  
  module.exports.tags = ["HarvestEthFactory"]
  module.exports.dependencies = ["ReceiptTokenFactory", "HarvestStrategyBeacon"]