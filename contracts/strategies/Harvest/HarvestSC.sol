...
import "../../interfaces/strategy/IInitializableHarvestSC.sol";
...
*/
contract HarvestSC is StrategyBase, HarvestBase, IInitializableHarvestSC,  ReentrancyGuardUpgradeable {
    ...
    function deposit(
        ...
        // validate
        // -----
        _validateDeposit(deadline, tokenAmount, totalInvested, slippage);
    ...
    function withdraw(
        ...
        // -----
        uint256 transferableRewards = _calculateRewards(msg.sender, amount, results.initialAmountfToken);
        (user.amountfToken, results.burnAmount) = _calculatefTokenRemainings(amount,results.initialAmountfToken);
        ...
        results.auctionedToken = 0;
          (results.feeableToken, results.earnedTokens) = _calculateFeeableTokens(results.initialAmountfToken,results.obtainedToken,user.amountToken,results.obtainedfToken,user.underlyingRatio);
        ...
        // final accounting
        // -----
        if (results.calculatedTokenAmount <= totalInvested) {
            totalInvested = totalInvested.sub(results.calculatedTokenAmount);
        } else {
            totalInvested = 0;
        }
